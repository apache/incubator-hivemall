	    ...
	    if (useSlackParameter) {
	      ...
	      for (int ti = 0; ti < numUniqueEvents; ti++) {
	        ...
	        for (int j = 0; j < ev.predIndexes.length; j++) {
	          ...
	          if (!modelExpects[pi].contains(di.getOMap().get(String.valueOf(x.getOutcome(ti))))) {
	            cfvalSum += 1;
	          }
	        }
...
*/
	  ...
	  private double nextIteration(int correctionConstant) {
	    ...
	    for (int ei = 0; ei < numUniqueEvents; ei++) {
	      ...
	      for (int j = 0; j < ev.predIndexes.length; j++) {
	        ...
	        if (predicateCounts[pi] >= cutoff) {
	          ...
	          for (int aoi=0;aoi<activeOutcomes.length;aoi++) {
	            ...
	            if (ev.values != null) {
	              modelExpects[pi].updateParameter(aoi,modelDistribution[oi] * ev.values[j] * 1);
	            }
	            else {
	      ...
	      loglikelihood += Math.log(modelDistribution[di.getOMap().get(String.valueOf(x.getOutcome(ei)))]) * 1;
	      numEvents += 1;
	      if (printMessages) {
	        int max = 0;
